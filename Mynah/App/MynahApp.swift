...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// The app was called Whispeur until 1.8.0. Models weigh up to a few GB, so the old
    /// support directory is moved rather than left orphaned and downloaded again.
    private func migrateSupportDirectoryFromWhispeur() {
        let fm = FileManager.default
        guard let support = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else { return }
        let old = support.appendingPathComponent("Whispeur", isDirectory: true)
        let new = support.appendingPathComponent("Mynah", isDirectory: true)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        migrateSupportDirectoryFromWhispeur()
