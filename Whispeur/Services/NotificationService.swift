...
private let logger = Logger(subsystem: "com.whispeur", category: "NotificationService")
...
final class NotificationService: NSObject, UNUserNotificationCenterDelegate {
    ...
    func sendRecordingDurationWarning(minutes: Int) async {
        ...
        content.body  = String(localized: "Whispeur enregistre depuis \(minutes) minutes. Pensez à arrêter la dictée si vous avez terminé.")
        content.sound = .default
        content.categoryIdentifier = Self.recordingCategoryIdentifier

        let request = UNNotificationRequest(
            identifier: "com.whispeur.recording-duration-\(minutes)min",
            content: content,
            trigger: nil
        )
        try? await center.add(request)
